...
- Powered by `langchain.chat_models.init_chat_model`; supports any provider keyed via `<provider>:<model>` strings (OpenAI, Anthropic, Google Gemini, xAI (Grok), OpenRouter, Ollama, HuggingFace, LiteLLM)
...
The `deep_think_llm` and `quick_think_llm` fields take a `<provider>:<model>` string. Use any provider supported by `langchain.chat_models.init_chat_model`: `openai:gpt-5`, `anthropic:claude-sonnet-4-6`, `google_genai:gemini-3-pro-preview`, `xai:grok-4`, `openrouter:meta-llama/llama-3`, `ollama:llama3`, `huggingface:meta-llama/...`, `litellm:gpt-5`, etc.
