...
點擊 [使用此模板](https://github.com/Mai0313/tradingagents/generate) 後即可開始。
...
1. **建立您的倉庫**：點擊 [使用此模板](https://github.com/Mai0313/tradingagents/generate) 建立新倉庫
...
3. **重新命名專案**：
    ...
    - 將 `src/tradingagents/` 目錄重新命名為 `src/your_project_name/`
    - 更新所有從 `tradingagents` 到 `your_project_name` 的匯入
...
```bash
# 若 console script 名稱為 "tradingagents"
uvx tradingagents
...
`pyproject.toml` 內提供 `tradingagents` 與 `cli` 兩個入口點。目前示範回傳簡單 `Response` 模型，可依需求替換。
