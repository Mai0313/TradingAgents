...
- **Publish Docker Image** (`build_image.yml`) — Builds and pushes Docker image to GHCR. Currently kept as a backup artifact only; the project is not deployed via Docker yet.
...
This section defines the Pydantic patterns used in this project. All classes under
`src/tradingagents/` must follow these conventions consistently.
